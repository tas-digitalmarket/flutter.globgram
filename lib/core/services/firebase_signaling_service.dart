...
class FirebaseSignalingService {
  ...
  void _handleSignalingMessage(SignalingMessage message) {
    _logger
        .debug('📬 Handling signaling: ${message.type} from ${message.from}');
    onMessage?.call(message);
  }

  /// Send an SDP offer to the room
  Future<void> sendOffer(
      RTCSessionDescription offer, String targetPeerId) async {
    try {
      final roomRef = _firestore.collection('rooms').doc(_roomId!);
  ...
  Future<void> sendIceCandidate(
      RTCIceCandidate candidate, String targetPeerId) async {
    try {
      final roomRef = _firestore.collection('rooms').doc(_roomId!);
